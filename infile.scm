...
(define sqrt-lzl 
  (lambda (x init)
     (if (= x init)
         (cons-lzl (cons x 0) (lambda () empty-lzl))
         (cons-lzl (cons init (abs (- x (square init))))
...
(define sqrt2
  (lambda (x init epsilon)
     (car (find-first (sqrt-lzl x init)
                 (lambda (guess) (< (abs (- (square (car guess)) x)) epsilon))))
  )
)
  ...
  (lambda (assoc-list key success fail)
     (if (empty? assoc-list)
         ...
         (if (eq? key (car (car assoc-list)))
             ...
             (get-value$ (cdr assoc-list) key
                         (lambda (succ-key) (success succ-key))
                        fail)))
  )
)
...
(define collect-all-values-1
 (lambda (lists key)
    (if (empty? lists)
        '()
        (if (eq? 'fail (get-value (car lists) key))
...
(define collect-all-values-2
 (lambda (lists key)
    (if (empty? lists)
        '()
        (if (eq? 'fail (get-value$ (car lists) key (lambda (x) x) (lambda () 'fail)))
                (collect-all-values-2 (cdr lists) key)
                (cons (get-value$ (car lists) key (lambda (x) x) (lambda () 'fail))
                      (collect-all-values-2 (cdr lists) key))))
 )
)
